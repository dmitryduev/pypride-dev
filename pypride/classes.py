...
class inp_set(object):
    ...
    def __init__(self, inp_file):
        ...
        self.abs_path = os.path.join(homedir, '.pypride')
        ...
        self.jpl_eph_cat = self.config.get('Ephemerides', 'jpl_eph_cat')
        ...
        if self.jpl_eph_cat[0] != '/':
            self.jpl_eph_cat = os.path.join(self.abs_path, self.jpl_eph_cat)
        ...
        self.sc_eph_force_update = self.config.getboolean('Switches', 'sc_eph_force_update')
        ...
        if section != 'all':
            if section in ('Catalogues', 'Ephemerides', 'Directories'):
                out = dict((k, os.path.join(self.abs_path, v)) for k, v in
                           self.config._sections[section].items() if k != '__name__')
            else:
...
class obs(object):
    ...
    def __str__(self):
        if self.inp['delay_calc']:
            echo = 'baseline: {:s}-{:s}, source: {:s}'.format(self.sta[0], self.sta[1], self.source)
    ...
    def addScan(self, start, step, stop=None, nobs=None, freq=None, force_step=False):
        # start and stop must be a datetime object
        # step is an integer time step in seconds

        if stop is None and nobs is None:
            raise Exception('You should specify either "stop" time or number of time stamps "nobs" to add')
        elif stop is None:
            # nobs is set
...
class site(object):
    ...
    def geodetic(self, const):
        ...
        self.lat_geod, self.h_geod = self.geoid(req, self.r_GTRS_date[2], AE, F)
    ...
    def AzEl2(self, gcrs, utc, JD, t_1, r2000, jpl_eph):
        ...
        mjd = JD - 2400000.5
        astropy_t_0 = Time(mjd + t_1 - lt_01 / 86400.0, format='mjd', scale='utc', precision=9)
    ...
    def addMet(self, date_start, date_stop, inp):
        ...
        if noSiteData:  # if met data are not there, load gridded data
            try:
                print('Meteo data for ' + self.name + ' not found. Using VMF1 grids instead.')
                ...
                for day in dates:
                    ...
                    vmf_grid_file = 'VMFG_{:4d}{:02d}{:02d}.H'.format(day.year, day.month, day.day)
                    ...
                    for hh in ('00', '06', '12', '18'):
                        ...
                        f = sp.interpolate.interp2d(lon, lat, ahz_grid.reshape((91, 144)))
                        ahz = f(self.lon_gcen, self.lat_geod)[0]
                        f = sp.interpolate.interp2d(lon, lat, awz_grid.reshape((91, 144)))
                        awz = f(self.lon_gcen, self.lat_geod)[0]
                        f = sp.interpolate.interp2d(lon, lat, zhz_grid.reshape((91, 144)))
                        zhz = f(self.lon_gcen, self.lat_geod)[0]
                        f = sp.interpolate.interp2d(lon, lat, zwz_grid.reshape((91, 144)))
        ...
        try:
            self.fMet['fAhz'] = sp.interpolate.interp1d(self.met['mjd'], self.met['ahz'], kind='cubic')
            self.fMet['fAwz'] = sp.interpolate.interp1d(self.met['mjd'], self.met['awz'], kind='cubic')
            self.fMet['fZhz'] = sp.interpolate.interp1d(self.met['mjd'], self.met['zhz'], kind='cubic')
            self.fMet['fZwz'] = sp.interpolate.interp1d(self.met['mjd'], self.met['zwz'], kind='cubic')
            if len(self.met['TC']) > 0:
                self.fMet['fT'] = sp.interpolate.interp1d(self.met['mjd'], self.met['TC'])
                self.fMet['fP'] = sp.interpolate.interp1d(self.met['mjd'], self.met['pres'])
                self.fMet['fH'] = sp.interpolate.interp1d(self.met['mjd'], self.met['hum'])
            if len(self.met['gnh']) > 0:
                self.fMet['fGnh'] = sp.interpolate.interp1d(self.met['mjd'], self.met['gnh'], kind='cubic')
                self.fMet['fGeh'] = sp.interpolate.interp1d(self.met['mjd'], self.met['geh'], kind='cubic')
...
class bindel(object):
    ...
    def getSources(self):
        ...
        """
        return list(set([self.scans[sn]['source'].strip() for sn in self.scans.keys()]))
        ...
        for sn in self.scans.keys():
            if self.scans[sn]['source'].strip() == source:
                ...
                try:
                    ...
                    ffit = optimalFit(ti, fi, min_order=3, max_order=7, fit_type='poly')
